...
MainWindow_::MainWindow_(QWidget *parent)
    : QMainWindow(parent), new_taille(nullptr), new_entree(nullptr), new_state(false),
      ...
      graphics_scene(nullptr), graphics_view(nullptr)
{
    ...
    std::vector<std::string> simul(existingLocalSims());
...
void MainWindow_::creer()
{
    ...
    graphics_view = new QGraphicsView(graphics_scene);
    //graphics_view = new QGraphicsView(graphics_scene, this);
    graphics_view->move(10, 90);
    graphics_view->resize(500, 500);
    graphics_view->setBackgroundBrush(QBrush(Qt::black, Qt::SolidPattern));
    graphics_view->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    graphics_view->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);     
    graphics_scene->setSceneRect(0, 0, 500, 500);
    graphics_scene->addRect(5, 5, 15, 15, QPen(Qt::blue));
    graphics_view->show();


    graphics_scene->setFocus();
}

void MainWindow_::creer_s()
{
    if (new_state)
    {
...
void MainWindow_::paintEvent(QPaintEvent *event)
{
    paint->begin(this);
    if (state == 1)
    {
...
bool MainWindow_::mouse_in(QMouseEvent* event)
{
    return ((event->x() >= 10 + d_x) && (event->x() <= int(ptr.px_x) + 10 - d_x))&&
           ((event->y() >= 90 + d_y) && (event->y() <= int(ptr.px_y) + 90 - d_y));
}

void MainWindow_::mousePressEvent(QMouseEvent *event)
{
            ...
            if (mouse_in(event))
            {
                ...
                if (calque.on_off)
                {
                    for (auto const& a : calque.alive)
                    {
                        ...
                        {
                            ptr.vue->addCell(pos.x() + a.first  - calque.translate.first,pos.y() + a.second - calque.translate.second);
                        }
                    }
                }
                else { ptr.vue->inverseCell(pos.x(), pos.y());}
                x_prec = x_current;
                y_prec = y_current;
                x_current = pos.x();
                y_current = pos.y();
...
void MainWindow_::mouseReleaseEvent(QMouseEvent* event)
{
...
void MainWindow_::mouseMoveEvent(QMouseEvent *event)
{
        ...
        if (mouse_in(event))
        {
            ...
            if ((event->buttons() == Qt::LeftButton && (x_prec != x_current || y_prec != y_current) && !simul_on) && !ctrl_on)
            {
                if (x_prec != -1 && y_prec != -1)
                {
                    ...
                    {
                        ptr.vue->addCell(a.first, a.second);
                    }
                }
...
void MainWindow_::wheelEvent(QWheelEvent* event)
{
    ...
    if (delta.y()%120 == 0)
    {
        ...
        */
        ...
        if (new_size >= 1 && new_size <= ptr.px_x && new_size <= ptr.px_y)
        {
            ...
            nb_lines = ptr.vue->nbLines();
            nb_col = ptr.vue->nbColumns();
...
bool MainWindow_::event(QEvent* event)
{
...
void MainWindow_::timerEvent(QTimerEvent *event)
{
    if (timer != 0)
    {
        Q_UNUSED(event);
...
void MainWindow_::keyPressEvent(QKeyEvent *event)
{
...
void MainWindow_::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event);
...
void MainWindow_::charger_calque()
{
...
void MainWindow_::charger_calques()
{
    calques->clear();
    calques->addItem("--select--");
...
void MainWindow_::calque_switch_s()
{
    if (calque.on_off)
    {
...
void MainWindow_::combo_time(int i)
{
    Q_UNUSED(i)
    if (frame_on &&
...
void MainWindow_::focus_frame(bool b)
{
...
void MainWindow_::item_changed_s(QString const& entree)
{
...
void MainWindow_::save_game_s()
{

}

void MainWindow_::reload_calques_s()
{
    charger_calques();
}
