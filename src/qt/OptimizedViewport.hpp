...
public:
	...
	void mouseDoubleClickEvent(QMouseEvent* event) {emit ViewportMouseDoucleClickEvent(event);}
	void toucheEvent(QTouchEvent* event) {emit ViewportTouchEvent(event);}   
...
signals:
	...
	void ViewportMouseDoucleClickEvent(QMouseEvent* event);
